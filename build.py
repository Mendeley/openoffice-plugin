    ...
    with zipfile.ZipFile('Mendeley-%s.oxt' % plugin_version, 'w') as extension_archive:
            ...
            for name in filenames:
                ...
                if name == 'description.xml':
                    with open(file_path) as description_file:
                        description_content = description_file.read().replace('%PLUGIN_VERSION%', plugin_version)
        ...
        for python_source_path in python_sources:
            with open(python_source_path) as python_source_file:
                plugin_python_script += python_source_file.read()
